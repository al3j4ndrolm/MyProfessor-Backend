...
def run_school_fetch_by_term(
    ...
    """
    Variant of `run_school_fetch` for fetching a single term. Unlike `run_school_fetch`,
    this skips saving school/terms data and skips setting the school status, and merges
    the newly fetched courses data into whatever is already stored in `courses` instead
    of overwriting it (new courses win on conflicting keys).
    """
    courses_data_table, classes_data_table = get_courses_and_classes()

    logger.info("Merging new courses data with existing data in database `courses`.")
