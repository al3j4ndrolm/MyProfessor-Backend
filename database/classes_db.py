...
def save(supabase: Client, classes_data_table: dict, school: str):
    ...
    for term, classes_all_departments in classes_data_table.items():
        for department, classes_per_department in classes_all_departments.items():
            ...
            if supabase.table(TABLE_NAME).select("*").eq("school", school).eq("department", department).eq("term", term).execute().data:
    ...
    for row in to_update:
        supabase.table(TABLE_NAME).update(row).eq("school", row["school"]).eq("department", row["department"]).eq("term", row["term"]).execute()
