...
from database.broadcasts_db import get, get_active
from database import db_keys

class TestBroadcastsDB:
    """Test class for broadcasts_db functions"""

    def test_get_returns_all_broadcasts(self):
        """Test that get returns all broadcast entries"""

        mock_supabase = Mock()
        mock_response = Mock()
        mock_response.data = [
            {"id": 1, "text": "Broadcast 1", "is_active": True},
            {"id": 2, "text": "Broadcast 2", "is_active": False},
        ]
