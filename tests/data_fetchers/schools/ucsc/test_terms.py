...
from data_fetchers.school_data.schools.ucsc import terms
...
TERM_NAME_PATTERN = re.compile(
    r"^\s*(\d{4}) (" + "|".join(SEASON_CODE_MAP.keys()) + r") Quarter\s*$"
)

class TestUCSCTerms(BaseSchoolTest):
    @property
    ...
    def test_get_terms(self):
        """Test getting terms from the live UCSC class search page"""
        ...
        for item in result:
            ...
            assert match, f"termName '{term_name}' does not match expected pattern '<YYYY> <Season> Quarter'"
